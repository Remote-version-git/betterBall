...
class DebugPlatform implements Platform {
  async addJifen(score) {
    ...
    var sign = window.md5(
      `${key}openid${window.playerInfo.openid}score${score}${timestamp}`
    );
    return this._request(
      `https://xwfintech.qingke.io/5f195b8dc01e13002c2d7341/openapi/pinball/add/measy?key=${key}&sign=${sign}&openid=${window.playerInfo.openid}&score=${score}&timestamp=${timestamp}`
    );
  }
  async getRank() {
    return this._request(
